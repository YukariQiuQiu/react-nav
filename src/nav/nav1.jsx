...
class Nav extends React.Component {
  ...
  UNSAFE_componentWillReceiveProps (){
    ...
    selectArr = [];
    this.searchId({path,list:navConfig})
    openArr = Array.from(new Set(openArr))
    selectArr = Array.from(new Set(selectArr))
    this.setState({selectedKeys:selectArr,openKeys:openArr})
  }
  ...
  searchId = ({path,list})=>{
    list.forEach((elem)=>{
      ...
      if(elem.path === path){
        ...
        // console.log(elem)
        openArr.push(elem.id)
        this.searchId({path,list:elem.child})
      }
    })
  ...
  showList = (key,path)=>{
    ...
    if(arr.indexOf(key) !== -1){
      let newSelect = []
      if(arr.length > 1) newSelect = arr.filter((elem) => {return elem !== key});
      openArr = JSON.parse(JSON.stringify(newSelect))
      setTimeout(()=>{this.setState({openKeys:newSelect})},100)
    } else {
  ...
  renderSubMenu = ({ name, path, child, id }) => {
    return (
      <SubMenu
        key={id}
        // onClick={(id)=>{this.showList(id)}}
        onTitleClick={()=>{this.showList(id,path)}}
        title={
          // <Link to={path}>
            <span>{name}</span>
          // </Link>
